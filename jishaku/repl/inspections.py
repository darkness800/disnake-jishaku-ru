...
def add_inspection(name):
    """
    Добавить проверку объекта Jishaku
    """

    # create the real decorator
    def inspection_inner(func):
        """
        Джишаку инспекционный декоратор
        """

        # Создайте инкапсулированную версию проверки, которая глотает исключения
...
def all_inspections(obj):
    """
    Генератор для итерации все текущие инспекции Джишаку.
    """
...
def class_name(obj):
    """
    Получите имя объекта, включая имя модуля, если доступно.
    """
...
def check_not_slot(obj, attr):
    """
    Убедитесь, что заданный атрибут - это не просто открытый слот для подклассов
    """
